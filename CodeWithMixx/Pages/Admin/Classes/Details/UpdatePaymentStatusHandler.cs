...
public class UpdatePaymentStatusHandler(AppDbContext context) : IHandler
{
    public async Task<Result> HandleAsync(int reservationId, decimal paidAmount, CancellationToken ct = default)
    {
        ...
        reservation.DiscountRate = CalculateDiscountRate(reservation.TotalPrice, pricePerClass, amountOfClasses);
        reservation.Bonus = CalculateBonus(reservation.TotalPrice, paidAmount);
        reservation.PaymentStatus = GetPaymentStatus(reservation.TotalPrice, paidAmount, classDate);
